...
def compute_grouped_ternary(w: mx.array, group_size: int = 128) -> tuple:
    """Grouped AbsMean Quantization: $W_g \in \mathbb{R}^{G}, \gamma_g = \frac{1}{G}\sum |W_g|$."""
    eps = 1e-5
    orig_shape = w.shape
...
def fused_ternary_matmul(x: mx.array, w_q: mx.array, gamma: mx.array, orig_shape: tuple, q_max: float = 127.0) -> mx.array:
    ...
    Maintains int8 in UMA; unpacks to float16 directly inside the execution register.
    """
    eps = 1e-5
    ...
    # Fused Dequantize & Matmul (Zero FP16 UMA expansion footprint)
